...
export async function geminiSupportReply(
  ...
  if (context.language === "ru") {
    systemInstruction = [
      ...
      "- Если клиент просит карту для оплаты или спрашивает, как оплатить:",
      `  • Давай реквизиты своей карты:`,
      `    💳 Карта для оплаты:`,
      `    ${cardNum}`,
      `    ${cardHolder}`,
      ...
      `  • ТОЛЬКО для опта и сотрудничества давай свой второй профиль: «По вопросам сотрудничества и опта напишите мне в профиль @${coopUser} — всё обсудим.»`,
      "",
    ...
    systemInstruction = [
      ...
      "  • Activation: After purchase, the customer receives an official Google link (serviceactivation.google.com). They open it, select their personal Gmail, and click 'Activate' / 'Accept'. No credit card or Google charges required (0 UZS).",
    ...
    systemInstruction = [
      ...
      "- Agar mijoz karta so'rasa yoki to'lov qilmoqchi bo'lsa:",
      `  • To'lov uchun kartangni ber:`,
      `    💳 To'lov uchun karta:`,
      `    ${cardNum}`,
      `    ${cardHolder}`,
      ...
      `  • FAQAT hamkorlik/optom so'ralganda ikkinchi shaxsiy profilingni ber: 'Hamkorlik va ulgurji savdo bo'yicha ikkinchi shaxsiy profilim @${coopUser} ga yozing, barcha shartlarni kelishamiz.'`,
      "",
