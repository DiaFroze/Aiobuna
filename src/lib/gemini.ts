...
export async function geminiSupportReply(
  ...
  if (context.language === "ru") {
    systemInstruction = [
      ...
      `    ${cardNum}`,
      `    ${cardHolder}`,
      `    После оплаты отправьте чек сюда в чат — я проверю и сразу выдам ссылку прямо в этот диалог. Также можете оформить автоматически через нашего бота @${botUser} (Click, Payme, Stars).`,
      ...
      "  • Отвечай: «Пришлите чек сюда в чат — я проверю платёж и выдам ссылку».",
