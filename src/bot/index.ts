...
bot.command("admin", async (ctx) => {
  ...
  const text =
    `🛠 <b>Панель администратора SubHub</b>\n\n` +
    `⚡ <b>Быстрая выдача товара:</b>\n` +
    `Откройте любой товар в каталоге — там доступна кнопка <i>«Выдать через админ»</i> (бесплатная выдача без оплаты).\n\n` +
    `👑 <b>Администраторы:</b>\n` +
    `• <b>/admins</b> — Список действующих администраторов\n` +
    `• <b>/addadmin &lt;tgId&gt;</b> — Назначить нового администратора\n` +
    `• <b>/deladmin &lt;tgId&gt;</b> — Отозвать права администратора\n` +
    `• <b>/myid</b> — Проверить свой Telegram ID и права\n\n` +
    `🛍 <b>Товары и оформление:</b>\n` +
    `• <b>/desc</b> (или <code>/setdesc</code>) — Настройка описания с любыми премиум-эмодзи\n` +
    `• <b>/order</b> — Очередность товаров в каталоге (кнопки ▲ / ▼)\n` +
    `• <b>/pmedia</b> — Прикрепить видео или баннер к товару\n` +
    `• <b>/banner</b> — Главный баннер магазина (приветствие)\n` +
    `• <b>/banner_reset</b> — Сбросить главный баннер\n` +
    `• <b>/stock</b> — Остатки цифровых ключей на складе\n\n` +
    `🎉 <b>Розыгрыши и Бусты канала:</b>\n` +
    `• <b>/giveaways</b> — Управление розыгрышами, проведение итогов и выбор победителей\n` +
    `• <b>/boost</b> — Бустер-хаб: автоматические скидки за буст вашего Telegram-канала\n\n` +
    `📢 <b>Рассылки и Акции:</b>\n` +
    `• <b>/post &lt;текст&gt;</b> — Массовая рассылка всем пользователям (или ответ командой /post на любое сообщение)\n` +
    `• <b>/promo</b> — Запуск глобальной скидочной акции\n` +
    `• <b>/promostop</b> — Остановка текущей промо-акции\n` +
    `• <b>/promopost</b> — Генерация готового рекламного поста акции для канала\n` +
    `• <b>/sendgifts</b> — Рассылка каталога подарков\n\n` +
    `💰 <b>Баланс и реферальная система:</b>\n` +
    `• <b>/give &lt;tgId&gt; &lt;сумма&gt;</b> — Пополнить баланс пользователя (в сумах)\n` +
    `• <b>/refs &lt;tgId&gt;</b> — Список приглашенных пользователя\n` +
    `• <b>/refinfo &lt;tgId&gt;</b> — Детальный аудит рефералов и заказов\n` +
    `• <b>/refgive &lt;tgId&gt; &lt;баллы&gt;</b> — Начислить реферальные баллы вручную\n` +
    `• <b>/reffix &lt;tgId&gt;</b> — Пересчет и синхронизация баллов\n` +
    `• <b>/refban &lt;tgId&gt;</b> / <b>/refunban</b> — Блокировка/разблокировка в реф. системе\n` +
    `• <b>/refzero &lt;tgId&gt;</b> — Обнуление реферальных баллов\n` +
    `• <b>/auditfraud</b> — Автоматический поиск подозрительных рефералов\n\n` +
    `⚙️ <b>Настройки и интеграции:</b>\n` +
    `• <b>/health</b> — Диагностика системы, БД и внешних шлюзов\n` +
    `• <b>/channels</b> — Проверка и привязка обязательных каналов для подписки\n` +
    `• <b>/salesgroup &lt;chatId&gt;</b> — Привязать группу для уведомлений о продажах\n` +
    `• <b>/course_setup</b> — Настройка закрытого канала курса AI\n` +
    `• <b>/terms</b> — Редактирование пользовательского соглашения\n` +
    `• <b>/review</b> / <b>/askreview</b> — Отзывы и запросы отзывов`;
  await ctx.reply(text, { parse_mode: "HTML" }).catch(() => {});
});
...
async function ensureSchema() {
  ...
  const statements = [
    ...
    `ALTER TABLE "AdExpense" ALTER COLUMN "adLinkId" DROP NOT NULL`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "metaCampaignId" TEXT`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "metaCampaignName" TEXT`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "metaAdSetId" TEXT`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "metaAdSetName" TEXT`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "metaAdId" TEXT`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "metaAdName" TEXT`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "metaSyncKey" TEXT`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "source" TEXT DEFAULT 'manual'`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "impressions" INTEGER DEFAULT 0`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "reach" INTEGER DEFAULT 0`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "clicks" INTEGER DEFAULT 0`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "cpm" DOUBLE PRECISION`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "cpc" DOUBLE PRECISION`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "ctr" DOUBLE PRECISION`,
    `ALTER TABLE "AdExpense" ADD COLUMN IF NOT EXISTS "syncedAt" TIMESTAMP(3)`,
    `CREATE INDEX IF NOT EXISTS "AdExpense_metaAdId_idx" ON "AdExpense"("metaAdId")`,
    `CREATE INDEX IF NOT EXISTS "AdExpense_metaCampaignId_idx" ON "AdExpense"("metaCampaignId")`,
    `CREATE INDEX IF NOT EXISTS "AdExpense_metaSyncKey_idx" ON "AdExpense"("metaSyncKey")`,
    `ALTER TABLE "AdLink" ADD COLUMN IF NOT EXISTS "metaCampaignId" TEXT`,
    `ALTER TABLE "AdLink" ADD COLUMN IF NOT EXISTS "metaAdSetId" TEXT`,
    `ALTER TABLE "AdLink" ADD COLUMN IF NOT EXISTS "metaAdId" TEXT`,
    `CREATE INDEX IF NOT EXISTS "AdLink_metaAdId_idx" ON "AdLink"("metaAdId")`,
